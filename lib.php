 ...
 * @return array
 */
function message_get_recent_conversations($user, $limitfrom = 0, $limitto = 100, $group = false)
{
    global $DB;
    ...
    //var_dump(['não lidas'=>$unread,'lidas'=>$read,'concatenadas',$unread+$read]);
    // exit();
...
function message_get_recent_notifications($user, $limitfrom = 0, $limitto = 100)
{
    ...
    $userfields = user_picture::fields('u', array('lastaccess'));
    $sql0 = "SELECT mr.id AS message_read_id, $userfields, 0 as new, mr.notification, mr.smallmessage, mr.fullmessage, mr.fullmessagehtml, mr.fullmessageformat, mr.timecreated as timecreated, mr.contexturl, mr.contexturlname
    ...
    $sql1 = "SELECT mr.id AS message_read_id, $userfields, 1 as new, mr.notification, mr.smallmessage, mr.fullmessage, mr.fullmessagehtml, mr.fullmessageformat, mr.timecreated as timecreated, mr.contexturl, mr.contexturlname
              FROM {message} mr
                   JOIN {user} u ON u.id=mr.useridfrom
             WHERE mr.useridto = :userid1 AND u.deleted = '0' AND mr.notification = :notification
             ORDER BY mr.timecreated DESC";

    $sql2 = "SELECT mr.id AS message_read_id, 0 as new, mr.notification, mr.smallmessage, mr.fullmessage, mr.fullmessagehtml, mr.fullmessageformat, mr.timecreated as timecreated, mr.contexturl, mr.contexturlname
              FROM {message_read} mr 
             WHERE mr.useridto = :userid1 AND mr.useridfrom=-10 AND mr.notification = :notification
             ORDER BY mr.timecreated DESC";

    $sql3 = "SELECT mr.id AS message_read_id, 1 as new, mr.notification, mr.smallmessage, mr.fullmessage, mr.fullmessagehtml, mr.fullmessageformat, mr.timecreated as timecreated, mr.contexturl, mr.contexturlname
              FROM {message} mr
             WHERE mr.useridto = :userid1  AND mr.useridfrom=-10 AND mr.notification = :notification
             ORDER BY mr.timecreated DESC";
    ...
    $notifications0 = $DB->get_records_sql($sql0, $params, $limitfrom, $limitto);
    $notifications1 = $DB->get_records_sql($sql1, $params, $limitfrom, $limitto);
    $notifications2 = $DB->get_records_sql($sql2, $params, $limitfrom, $limitto);
    $notifications3 = $DB->get_records_sql($sql3, $params, $limitfrom, $limitto);

    $notifications = array_merge($notifications0, $notifications1, $notifications2, $notifications3);
 ...
 */
function message_print_recent_conversations($user1 = null, $showicontext = false, $showactionlinks = true, $mobile = false)
{
    global $USER;
    ...
    $showotheruser = true;
    message_print_recent_messages_table($conversations, $user1, $showotheruser, $showicontext, false, $showactionlinks, $mobile);
}
 ...
 * @return void
 */
function message_print_recent_messages_table($messages, $user = null, $showotheruser = true, $showicontext = false, $forcetexttohtml = false, $showactionlinks = true, $mobile = false)
{
    global $OUTPUT, $USER;
    static $dateformat;
    $total = 0;
    ...
    foreach ($messages as $message) {
        if (isset($message->id)) {
            $total = message_count_unread_messages($USER, $message);
        }

        $total = $total > 0 ? html_writer::div($total, 'badge badge-theme pull-right') : "";
        if ($mobile) {
            $message->contexturl = new moodle_url2($message->contexturl, ['m' => '3']);
            $message->contexturl->setPath(str_replace('index','mobile',$message->contexturl->get_path()));
        }
        echo html_writer::start_tag('a', [
            'class' => 'list-group-item',
            'href' => $message->contexturl,
            'title' => fullname($message) . " em " . userdate($message->timecreated, $dateformat) . ":\n " . strip_tags($message->smallmessage)
        ]);
        echo html_writer::start_tag('div', array('class' => 'notification-img'));
        echo html_writer::img(new moodle_url("/user/pix.php/" . (isset($message->id) ? $message->id : '1') . "/f2.jpg"),
            $message->contexturlname, ['class' => 'userpicture']);
        echo html_writer::end_tag('div');
        echo html_writer::start_tag('div', array('class' => 'notification-item'));
        echo html_writer::tag('h4', "<b>" . fullname($message) . "</b> " . $total, ['class' => 'item-title']);
        echo html_writer::tag('p', nl2br(strip_tags($message->smallmessage)) . "<br>"
            . html_writer::tag('small',
                html_writer::tag('b', html_writer::tag('i', null, ['class' => 'fa fa-clock-o'])) . " "
                . userdate($message->timecreated, $dateformat)), ['class' => 'item-info']);
        echo html_writer::end_tag('div');
...
function message_print_search_results($frm, $showicontext = false, $currentuser = null)
{
    ...
    if ($personsearch) {
        ...
        if (!empty($users)) {
            echo html_writer::start_tag('div', array('class' => 'panel'));
            echo html_writer::start_tag('div', array('class' => 'panel-heading searchresultcount'));
            ...
            foreach ($users as $user) {
                ...
                echo html_writer::start_tag('td', array('widith' => '3%'));
                echo $OUTPUT->user_picture($user, array('size' => 32, 'courseid' => SITEID));
            ...
            echo html_writer::div(get_string('userssearchresults', 'message', 0), 'alert alert-info searchresultcount');
        }
    }
    ...
    if ($messagesearch) {
        ...
        if (($messages = message_search($keywords, $fromme, $tome, $courseid)) !== false) {
            ...
            echo html_writer::start_tag('div', array('class' => 'panel-body searchresultcount'));
            // Print table headings.
            echo html_writer::start_tag('table', array('class' => 'table table-hover messagesearchresults'));
            ...
            echo html_writer::div(get_string('keywordssearchresults', 'message', 0), 'alert alert-info searchresultcount');
        }
    }
 ...
 * @param bool $viewingnewmessages are we currently viewing new messages?
 */
function message_get_history2($user1, $user2, $limitnum = 0, $viewingnewmessages = false)
{
    global $DB, $CFG;
 ...
 * @param bool $viewingnewmessages are we currently viewing new messages?
 */
function message_print_message_history($user1, $user2, $search = '', $messagelimit = 0, $messagehistorylink = false, $viewingnewmessages = false, $showactionlinks = true, $mobile = false)
{
    global $OUTPUT, $PAGE;
    ...
    if ($messages = message_get_history($user1, $user2, $messagelimit, $viewingnewmessages)) {
        ...
        foreach ($messages as $message) {
            ...
            $pixopt = ['size' => $mobile ? 24 : 48, 'courseid' => SITEID,'tabindex'=>-1];

            if ($message->useridfrom == $user1->id) {
                $formatted_message = message_format_message($message, $messagedate, $search,
                    $mobile ? 'notification-item me' : 'col-md-offset-4 col-xs-10  col-sm-8  me col-md-6');
                $formatted_message .= html_writer::div($OUTPUT->user_picture($user1, $pixopt), $mobile ? 'notification-img' : 'img-me');
                $side = '';
            } else {
                $formatted_message = html_writer::div($OUTPUT->user_picture($user2, $pixopt), $mobile ? 'notification-img' : 'img-other');
                $formatted_message .= message_format_message($message, $messagedate, $search,
                    $mobile ? 'notification-item other' : 'col-xs-10 col-sm-9 col-md-6  other');
                $side = '';
            }
            $side =$mobile?'mobile':'row';
            ...
            if (message_can_delete_message($message, $user1->id)) {
                ...
                $deleteurl = new moodle_url($mobile?'/message/mobile.php':'/message/index.php', array('user1' => $user1->id, 'user2' => $user2->id,
                ...
                $deleteicon = html_writer::link($deleteurl, get_string('delete'), ['class' => 'btn btn-block btn-danger']);
                $deleteicon = html_writer::tag('div', $deleteicon, array('class' => 'col-md-2 deleteicon accesshide'));
                $formatted_message .= $deleteicon;
            }

            $tablecontents .= html_writer::tag('div', $formatted_message, array('class' => "messagecontent $side $notificationclass", 'id' => 'message_' . $messagenumber));

        }
        echo html_writer::nonempty_tag('div', $tablecontents, array('class' => ' messagehistory'));
    } else {
...
function message_format_message($message, $format = '', $keywords = '', $class = 'other')
{
    ...
    // Remove as firulinhas do usuários (cor)
    //$messagetext = preg_replace('/(<[^>]+) style=".*?"/i', '$1', $messagetext);
    $messageC = strip_tags($messagetext);

    return <<<TEMPLATE
<div class='message $class' title='$messageC' role='button'>
    <a name="m{$message->id}"></a> 
    <span class="message-meta"><span class="time"><i class="fa fa-clock-o" aria-hidden="true"></i> $time</span></span> <span class="text">$messagetext</span>
</div>
TEMPLATE;
}
...
function message_print_contactlist_user($contact, $incontactlist = true, $isblocked = false, $selectcontacturl = null, $showactionlinks = true, $selecteduser = null)
{
    ...
    if (!empty($selectcontacturl)) {
        $link = new moodle_url($selectcontacturl . '&history=1&user2=' . $contact->id);
    } else {
        //can $selectcontacturl be removed and maybe the be removed and hardcoded?
        $link = new moodle_url("/message/index.php?id=$contact->id&history=1");
...
/**
 * Implementa a função setPath
 * Class moodle_url2
 */
class moodle_url2 extends moodle_url
{
    /**
     * @param string $path
     */
