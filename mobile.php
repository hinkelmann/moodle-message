...
$m                    = optional_param('m', 1, PARAM_INT);
...
$url = new moodle_url('/message/mobile.php', array('user1' => $user1id));
...
if ($addcontact and confirm_sesskey()) {
    ...
    redirect($CFG->wwwroot . '/message/mobile.php?viewing=contacts&id=' . $addcontact);
}
    ...
    if ($message = $DB->get_record($deletemessagetype, array('id' => $deletemessageid))) {
        ...
        if (message_can_delete_message($message, $user1->id)) {
            if (!$deletemessageconfirm) {
                $confirmurl = new moodle_url('/message/mobile.php', array('user1' => $user1->id, 'user2' => $user2->id,
    ...
    if (empty($messageerror)) {
        ...
        if (!empty($data) && !empty($data->message)) {
            ...
            if (!empty($messageid)) {
                ...
                redirect($CFG->wwwroot . '/message/mobile.php?viewing=' . $viewing . '&id=' . $user2->id);
            }
        }
    }
}
...
$PAGE->set_title($SITE->shortname . ":" . get_string('messages', 'message'));
$usernode = $PAGE->navigation->find('users', null)->remove();
...
switch ($m) {
    case 1:
        ...
        message_print_recent_conversations($user1, false, $showactionlinks, true);
        break;
    case 2:
    ...
    case 3:
        ...
        if ($user1 and $user2) {
            message_print_message_history($user1, $user2, $search, 0, '', '', $showactionlinks, true);
